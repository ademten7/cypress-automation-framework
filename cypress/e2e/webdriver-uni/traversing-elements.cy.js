...
describe("Traversing DOM elements in Cypress", () => {
  beforeEach(() => {
    ...
    it("closest() to validate the closest ancestor(parent) DOM element", () => {
    ...
    it("filter() to retrieve DOM elements that match a specific selector", () => {
      cy.get('button[type=button]').filter('.active').should('contain','Button-1').click().should('have.class','focus')
    });
  
    it("find() to retrieve DOM elements of a given selector", () => {
      cy.get('.traversal-pagination').find('li').find('.page-link').should('have.length',7)
    });
